...
studentdata.sp = st_intersection(studentdata.f, philly %>% dplyr::select("name")) %>% rename(tractName = name)
...
reg2 <- lm(sale_price ~ ., data = studentdata.sp %>% st_drop_geometry() %>% 
                                 dplyr::select(sale_price, age, totRooms, incBasements, incAC, buildingQuality, incGarage, stories, scenery, maxArea, farmersmarket.d.ft, schools.c, hospital.d.ft, bikestations.c, parks.d.ft, minority.m, medHHInc, tractName))
...
trainingReg.sp <- lm(sale_price ~ ., na.action = na.omit, data = trainingmodel.sp %>% 
                      st_drop_geometry() %>% 
                    dplyr::select(sale_price, age, totRooms, incBasements, incAC, incGarage, buildingQuality, stories, maxArea, farmersmarket.d.ft, schools.c, hospital.d.ft, bikestations.c, parks.d.ft, minority.m, medHHInc, tractName))
...
```{r}
training.coords <-  st_coordinates(testingmodel.sp.m) 
training.tracts <- knn2nb(knearneigh(training.coords, 5))
spatialWeights.test_n <- nb2listw(training.tracts, style="W")

moranTest_n <- moran.mc(testingmodel.sp.m$SalePrice.Error, 
